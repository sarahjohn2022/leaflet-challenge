...
function makeMap(earthquake_data)
{
    let myMap = L.map("map", 
    {
    ...
    {
        let earthquake = earthquake_data[i];
        L.circle([earthquake.geometry.coordinates[1], earthquake.geometry.coordinates[0]], 
          ...
          .bindPopup(`<h3>${earthquake.properties.place}</h3><hr><p>Magnitude: ${earthquake.properties.mag}<br>Depth: ${earthquake.geometry.coordinates[2]}</p>`).addTo(myMap);
    }
